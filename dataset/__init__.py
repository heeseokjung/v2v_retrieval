...
from dataset.activitynet import (
    ActivityNetCaptionsRetrievalTrainDataset,
    ActivityNetCaptionsRetrievalEvalDataset,
)
...
class ActivityNetCaptionsCollator(object):
    def __init__(self, is_train, model):
        self.is_train = is_train
        self.model = model
...
def get_train_dataset(cfg):
    if cfg.DATASET.name == "moma":
        ...
        dataset = ActivityNetCaptionsRetrievalTrainDataset(cfg)
        collate_fn = ActivityNetCaptionsCollator(is_train=True, model=cfg.MODEL.name)
        return dataset, collate_fn
    else:
